// components/admin/SellerDetail.jsx
...
const SellerDetail = () => {
    ...
    const handleEditSeller = () => {
        navigate(`/admin/dashboard/sellers/edit/${sellerId}`);
    };
    ...
    const handleStatusToggle = async (checked) => {
        try {
            ...
            // await dispatch(adminUpdateSellerStatus({ sellerId, isActive: checked })).unwrap();
            message.success(`Seller ${checked ? 'activated' : 'deactivated'} successfully`);
        } catch (error) {
    ...
    const calculateSellerStats = () => {
        ...
        // Product statistics
        const totalStock = sellerProducts.reduce((sum, product) => sum + (product.stock || 0), 0);
        const totalSold = sellerProducts.reduce((sum, product) => sum + (product.sold || 0), 0);
            ...
            render: (_, record) => (
                <Space size="small">
                    ...
                    <Button
                        size="small"
                        icon={<EditOutlined />}
                        onClick={() => navigate(`/admin/dashboard/products/edit/${record._id}`)}
                    >
                        Edit
                    </Button>
                </Space>
                                ...
                                <Card title="Product Statistics">
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <Progress
                                                percent={Math.min(100, (stats.totalStock / (stats.totalStock + stats.totalSold)) * 100).toFixed(2)}
                                                status="active"
                                            />
                                        </div>
                                ...
                                <Card title="Recent Activity">
                                    <List
                                        ...
                                        renderItem={(order) => (
                                            <List.Item>
                                                <List.Item.Meta
                                                    avatar={<ShoppingOutlined className="text-green-500" />}
                                                ...
                                                <div>
                                                    {seller.address ? (
                                                        <>
                                                            ...
                                                            <div>{seller.address.city}, {seller.address.state}</div>
                                                            <div>{seller.address.zipCode}, {seller.address.country}</div>
                                                        </>
                                                    ) : (
                                                        'Not provided'
                                                    )}
