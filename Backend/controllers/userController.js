...
export const login = asyncHandler(async (req, res) => {
  ...
  const accessToken = jwt.sign({ id: find._id }, process.env.ACCESS_KEY, {
    expiresIn: "15m",
  });
  const refreshToken = jwt.sign({ id: find._id }, process.env.REFRESH_KEY, {
    expiresIn: "7d",
  });
...
export const refreshToken = asyncHandler(async (req, res) => {
  ...
  const newRefreshToken = jwt.sign({ id: user._id }, process.env.REFRESH_KEY, {
    expiresIn: "7d",
  });
...
export const getUser = asyncHandler(async (req, res) => {
  ...
  const [users, totalUsers] = await Promise.all([
    User.find({}, "name email age role createdAt avatar ordersCount")
      .skip(skip)
      .limit(limit),
    User.countDocuments(),
  ]);
...
export const profile = asyncHandler(async (req, res) => {
  ...
  const user = await User.findById(id)
    .select(
      "name email age role cart addresses avatar createdAt updatedAt ordersCount"
    )
...
export const updateCart = asyncHandler(async (req, res) => {
  ...
  if (!productId || !count)
    return res
      .status(400)
      .json({ message: "productId and count are required" });
  ...
  if (!user)
    return res
      .status(404)
      .json({ message: "User not found or product not in cart" });
...
export const getAllSeller = asyncHandler(async (req, res) => {
  ...
  const sellers = await User.find({ role: "seller" })
    .select("name email role storeName productsCount isActive createdAt")
    .skip(skip)
    .limit(limit);
